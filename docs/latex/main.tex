...
\begin{tabular}{lccc}
\hline
$s$ & \textbf{Learned static} & \textbf{ACS + var.\ density} & \textbf{Gain} \\
\hline
0.25 & \textbf{0.02355} & 0.02450 $\pm$ 0.00006 & $+3.79\%$ \\
0.40 & \textbf{0.01842} & 0.01873 $\pm$ 0.00005 & $+2.14\%$ \\
0.50 & \textbf{0.01521} & 0.01538 $\pm$ 0.00007 & $+1.49\%$ \\
0.75 & 0.00763 & 0.00764 $\pm$ 0.00001 & $+0.18\%$ \\
\hline
\end{tabular}
\caption{Held-out val NMSE under the judge U-Net B. The first three gains exceed $2$ seed standard deviations; $s=0.75$ is a tie.}
\label{tab:mri-main}
\end{table}

It is the best mask tested at every sparsity, and it also beats variable density under plain zero-filled reconstruction at three of four sparsities, so it is not exploiting one reconstructor's quirks. The gap between the relaxed and combinatorial optimizers is not small: the relaxed profile lost by $1.5$--$2.3\%$ where the discrete search won by $1.5$--$3.8\%$, on the same objective and data.
