...
\begin{tabular}{lcccc}
\hline
$s$ & \textbf{Learned static} & \textbf{ACS + var.\ density} & \textbf{Gain} & \textbf{LOUPE} \\
\hline
...
\hline
\end{tabular}
\caption{Held-out val NMSE under the judge U-Net B. The first three gains over variable density exceed $2$ seed standard deviations; $s=0.75$ is a tie. LOUPE \citep{bahadir2020loupe}: row-restricted, trained to mask convergence, budget-exact top-$k$.}
\label{tab:mri-main}
\end{table}
